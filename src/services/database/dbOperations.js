...
module.exports = {
  ...
  getPrivateChatBetweenUsers: async (user1Id, user2Id) => {
    ...
    if (chat) {
      chat.participants = [user1Id, user2Id];
      chat.display_name = await module.exports.getUsernameById(user2Id);
    }
  ...
  getUsernameById: async (userId) => {
    const user = await db.one("SELECT username FROM users WHERE user_id = $1", [
      userId,
    ]);
